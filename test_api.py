...
# 基础URL
BASE_URL = "https://k5.vicicode.cn/api"
...
def test_lol_api():
    ...
    # 设置缓存
    res0 = requests.post(url, json={"func": 0, "uuid":"test_cache", "cache":"[state.lng, state.lat, state.alt]"})
    print(f"设置缓存 | 状态码：{res0.status_code} | 响应：{res0.text}")

    # 获取缓存
    res1 = requests.post(url, json={"func": 1, "uuid":"test_cache"})
